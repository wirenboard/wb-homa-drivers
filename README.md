...
Поле mux_channel_number указывает к какому каналу ADC относится описание канала. Порядок вывода на публикацию в MQTT соответствует порядку, 
в котором каналы введены в конфиге.
