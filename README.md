...
те гпио, с которыми драйвер должен работать, и их описания. Возможные параметры гпио можно посмотреть в файлах, названия которых начинаются на 
`/etc/wb-homa-gpio.conf.`. Также драйвер работает с гпио, поддерживающими прерывания, для таких гпио можно указать дополнительно параметры
```
{
    // указать только если к гпио подключен счетчик, для водных счетчиков "water_meter", для счетчиков электроэнергии "watt_meter"
    "type" : "",
    ...
    // для гпио с type = "" по умолчанию устанавливается both, для счетчиков если не указан, 
    // вычисляется в момент запуска, посмотрев несколько раз значение ../gpioNUM/vale у соответсвующего гпио
    "edge" : "falling"
}
```
