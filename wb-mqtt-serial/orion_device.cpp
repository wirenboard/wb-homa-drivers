...
REGISTER_BASIC_INT_PROTOCOL("orion", TOrionDevice, TRegisterTypes(
        {
            ...
            { TOrionDevice::REG_RELAY_MULTI, "relay_multi", "holding", U8, true },
            { TOrionDevice::REG_RELAY_DEFAULT, "relay_default", "holding", U8, true },
            { TOrionDevice::REG_RELAY_DELAY, "relay_delay", "holding", U8, true }
        }));
...
uint8_t TOrionDevice::CrcOrion(const uint8_t *array, int size)
{
    uint8_t crc = 0;
...
void TOrionDevice::WriteRegister(PRegister reg, uint64_t value)
{
    ...
    if (response[5] != CrcOrion(response, 5)) {
